*k2-native-items.lua*

Items introduced by Krastorio2. Their original recipes for reference and RX overwrites.
Recipe name is the same as the resulting item name.

---

K2 [[recipe_kr-tree-seed]] :⏳2s, 📚200/100
→ 2 [[item_wood]] 
← 1 [[item_tree-seed]] 

---

RX [[recipe_kr-tree-seed]] :⏳12s, 📚200/2000
→ 24 [[item_rx-wood-chips]] 
← 1 [[item_tree-seed]] 

---

K2 [[recipe_kr-iron-beam]] :⏳2s, 📚200/200
→ 2 [[item_iron-plate]] 
← 1 [[item_kr-iron-beam]] :

---
K2 [[recipe_kr-steel-beam]] :⏳2s, 📚200/200
→ 2 [[item_steel-plate]] 
← 1 [[item_kr-steel-beam]]

---
  K2 [[recipe_kr-steel-gear-wheel]] :⏳0.5s, 📚200/200
→ 1 [[item_steel-plate]] 
← 1 [[item_kr-steel-gear-wheel]]

---

K2 [[recipe_kr-automation-core]]:⏳5s, 📚200/400
→ 6 [[item_copper-plate]] + 4 [[item_iron-gear-wheel]] + 4 [[item_iron-stick]]
← 2 [[item_kr-automation-core]]

---

K2 [[recipe_kr-electronic-components]]:⏳4s, 📚200/400
→ 2 [[item_kr-glass]] + 2 [[item_kr-silicon]] + 4 [[item_plastic-bar]]
← 2 [[item_kr-electronic-components]]

---

K2 [[recipe_kr-inserter-parts]]:⏳1s, 📚200/200
→ 2 [[item_iron-gear-wheel]] + 2 [[item_iron-stick]]
← 1 [[item_kr-inserter-parts]]

---

